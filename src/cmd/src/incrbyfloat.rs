...
impl Cmd for IncrbyFloatCmd {
    ...
    fn execute_typed(&self, client: &Client, storage: Arc<Storage>) -> Option<CommandResult> {
        let argv = client.argv();
        Some(match argv.len() {
            3 => match String::from_utf8_lossy(&argv[2]).parse::<f64>() {
                Ok(increment) if increment.is_finite() => {
                    match storage.incr_decr_float(&argv[1], increment) {
                        Ok(value) => {
                            let formatted = if value.fract() == 0.0 && value.abs() < i64::MAX as f64
                            {
                                format!("{value:.0}")
                            } else {
                                value.to_string()
                            };
                            Ok(RespData::BulkString(Some(formatted.into())))
                        }
