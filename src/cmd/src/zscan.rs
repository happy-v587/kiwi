// Copyright (c) 2024-present, arana-db Community.  All rights reserved.
//
// Licensed to the Apache Software Foundation (ASF) under one or more
// contributor license agreements.  See the NOTICE file distributed with
// this work for additional information regarding copyright ownership.
// The ASF licenses this file to You under the Apache License, Version 2.0
// (the "License"); you may not use this file except in compliance with
// the License.  You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

use std::sync::Arc;

use client::Client;
use resp::RespData;
use storage::storage::Storage;

use crate::{AclCategory, ClientExt, Cmd, CmdFlags, CmdMeta};
use crate::{impl_cmd_clone_box, impl_cmd_meta};

#[derive(Clone, Default)]
pub struct ZscanCmd {
    meta: CmdMeta,
}

impl ZscanCmd {
    pub fn new() -> Self {
        Self {
            meta: CmdMeta {
                name: "zscan".to_string(),
                arity: -3, // ZSCAN key cursor [MATCH pattern] [COUNT count]
                flags: CmdFlags::READONLY | CmdFlags::FAST,
                acl_category: AclCategory::SORTEDSET | AclCategory::READ,
                ..Default::default()
            },
        }
    }
}

impl Cmd for ZscanCmd {
    impl_cmd_meta!();
    impl_cmd_clone_box!();

    fn do_initial(&self, client: &Client) -> bool {
        let argv = client.argv();
        if argv.len() < 3 {
            return false;
        }

        // Set the key for locking
        let key = argv[1].clone();
        client.set_key(&key);
        true
    }

    fn do_cmd(&self, client: &Client, storage: Arc<Storage>) {
        let argv = client.argv();

        let key = argv[1].as_slice();
        let cursor_str = String::from_utf8_lossy(&argv[2]);

        // Parse cursor
        let cursor = match cursor_str.parse::<u64>() {
            Ok(c) => c,
            Err(_) => {
                client.set_error(error_catalog::INVALID_CURSOR);
                return;
            }
        };

        // Parse optional MATCH and COUNT parameters
        let mut pattern: Option<String> = None;
        let mut count: Option<usize> = None;

        let mut i = 3;
        while i < argv.len() {
            let arg = String::from_utf8_lossy(&argv[i]).to_uppercase();
            match arg.as_str() {
                "MATCH" if i + 1 < argv.len() => {
                    pattern = Some(String::from_utf8_lossy(&argv[i + 1]).to_string());
                    i += 2;
                }
                "MATCH" => {
                    client.set_error(error_catalog::SYNTAX_ERROR);
                    return;
                }
                "COUNT" if i + 1 < argv.len() => {
                    match String::from_utf8_lossy(&argv[i + 1]).parse::<usize>() {
                        Ok(c) if c > 0 => {
                            count = Some(c);
                            i += 2;
                        }
                        _ => {
                            client.set_error(error_catalog::VALUE_NOT_INTEGER);
                            return;
                        }
                    }
                }
                "COUNT" => {
                    client.set_error(error_catalog::SYNTAX_ERROR);
                    return;
                }
                _ => {
                    client.set_error(error_catalog::SYNTAX_ERROR);
                    return;
                }
            }
        }

        let result = storage.zscan(key, cursor, pattern.as_deref(), count);

        match result {
            Ok((next_cursor, members_scores)) => {
                // Convert member-score pairs to RespData
                // ZSCAN returns alternating member and score values
                let mut resp_items = Vec::new();
                for (member, score) in members_scores {
                    resp_items.push(RespData::BulkString(Some(member.into_bytes().into())));
                    resp_items.push(RespData::BulkString(Some(score.into_bytes().into())));
                }

                // Return [next_cursor, [member1, score1, member2, score2, ...]]
                let response = vec![
                    RespData::BulkString(Some(next_cursor.to_string().into_bytes().into())),
                    RespData::Array(Some(resp_items)),
                ];

                client.set_reply(RespData::Array(Some(response)));
            }
            Err(e) => {
                client.set_storage_error(&e);
            }
        }
    }
}

#[allow(clippy::unwrap_used)]
#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_zscan_cmd_meta() {
        let cmd = ZscanCmd::new();
        assert_eq!(cmd.name(), "zscan");
        assert_eq!(cmd.meta().arity, -3); // ZSCAN key cursor [MATCH pattern] [COUNT count]
        assert!(cmd.has_flag(CmdFlags::READONLY));
        assert!(cmd.has_flag(CmdFlags::FAST));
        assert!(!cmd.has_flag(CmdFlags::WRITE));
    }

    #[test]
    fn test_zscan_cmd_clone() {
        let cmd = ZscanCmd::new();
        let cloned = cmd.clone_box();
        assert_eq!(cloned.name(), cmd.name());
        assert_eq!(cloned.meta().arity, cmd.meta().arity);
    }

    #[test]
    fn test_zscan_acl_category() {
        let cmd = ZscanCmd::new();
        assert!(cmd.acl_category().contains(AclCategory::SORTEDSET));
        assert!(cmd.acl_category().contains(AclCategory::READ));
    }

    #[test]
    fn test_zscan_argument_validation() {
        let cmd = ZscanCmd::new();

        // Valid argument counts
        assert!(cmd.check_arg(3)); // ZSCAN key cursor
        assert!(cmd.check_arg(5)); // ZSCAN key cursor MATCH pattern
        assert!(cmd.check_arg(7)); // ZSCAN key cursor MATCH pattern COUNT count

        // Invalid argument counts
        assert!(!cmd.check_arg(1)); // Missing key and cursor
        assert!(!cmd.check_arg(2)); // Missing cursor
        assert!(!cmd.check_arg(0)); // No arguments
    }
}
