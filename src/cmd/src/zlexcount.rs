...
impl Cmd for ZlexcountCmd {
    ...
    fn do_cmd(&self, client: &Client, storage: Arc<Storage>) {
        ...
        // Perform the ZLEXCOUNT operation
        match storage.zlexcount(key, min, max) {
            Ok(count) => {
    ...
    fn execute_typed(&self, client: &Client, storage: Arc<Storage>) -> Option<CommandResult> {
        ...
        Some(match argv.len() {
            4 => storage
                .zlexcount(&argv[1], &argv[2], &argv[3])
