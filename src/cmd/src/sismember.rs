// Copyright (c) 2024-present, arana-db Community.  All rights reserved.
//
// Licensed to the Apache Software Foundation (ASF) under one or more
// contributor license agreements.  See the NOTICE file distributed with
// this work for additional information regarding copyright ownership.
// The ASF licenses this file to You under the Apache License, Version 2.0
// (the "License"); you may not use this file except in compliance with
// the License.  You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

use std::sync::Arc;

use client::Client;
use resp::RespData;
use storage::storage::Storage;

use crate::{AclCategory, ClientExt, Cmd, CmdFlags, CmdMeta, CommandResult};
use crate::{impl_cmd_clone_box, impl_cmd_meta};

#[derive(Clone, Default)]
pub struct SismemberCmd {
    meta: CmdMeta,
}

impl SismemberCmd {
    pub fn new() -> Self {
        Self {
            meta: CmdMeta {
                name: "sismember".to_string(),
                arity: 3, // SISMEMBER key member
                flags: CmdFlags::READONLY | CmdFlags::FAST,
                acl_category: AclCategory::SET | AclCategory::READ,
                ..Default::default()
            },
        }
    }
}

impl Cmd for SismemberCmd {
    impl_cmd_meta!();
    impl_cmd_clone_box!();

    fn do_initial(&self, client: &Client) -> bool {
        let argv = client.argv();
        let key = argv[1].clone();
        client.set_key(&key);
        true
    }

    fn do_cmd(&self, client: &Client, storage: Arc<Storage>) {
        let key = client.key();
        let argv = client.argv();
        let member = &argv[2];

        let result = storage.sismember(&key, member);

        match result {
            Ok(is_member) => {
                client.set_reply(RespData::Integer(if is_member { 1 } else { 0 }));
            }
            Err(e) => {
                client.set_storage_error(&e);
            }
        }
    }

    fn execute_typed(&self, client: &Client, storage: Arc<Storage>) -> Option<CommandResult> {
        let argv = client.argv();
        Some(match argv.len() {
            3 => storage
                .sismember(&argv[1], &argv[2])
                .map(|is_member| RespData::Integer(i64::from(is_member)))
                .map_err(crate::error::CommandError::storage),
            _ => Err(crate::error::CommandError::WrongArity {
                command: self.name().to_string(),
            }),
        })
    }

    fn uses_typed_execution(&self) -> bool {
        true
    }
}

#[allow(clippy::unwrap_used)]
#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_sismember_cmd_meta() {
        let cmd = SismemberCmd::new();
        assert_eq!(cmd.name(), "sismember");
        assert_eq!(cmd.meta().arity, 3); // SISMEMBER key member
        assert!(cmd.has_flag(CmdFlags::READONLY));
        assert!(cmd.has_flag(CmdFlags::FAST));
        assert!(!cmd.has_flag(CmdFlags::WRITE));
    }

    #[test]
    fn test_sismember_cmd_clone() {
        let cmd = SismemberCmd::new();
        let cloned = cmd.clone_box();
        assert_eq!(cloned.name(), cmd.name());
        assert_eq!(cloned.meta().arity, cmd.meta().arity);
    }

    #[test]
    fn test_sismember_acl_category() {
        let cmd = SismemberCmd::new();
        assert!(cmd.acl_category().contains(AclCategory::SET));
        assert!(cmd.acl_category().contains(AclCategory::READ));
    }

    #[test]
    fn test_sismember_argument_validation() {
        let cmd = SismemberCmd::new();

        // Valid argument count (3: command + key + member)
        assert!(cmd.check_arg(3));

        // Invalid argument counts
        assert!(!cmd.check_arg(1)); // Missing key and member
        assert!(!cmd.check_arg(2)); // Missing member
        assert!(!cmd.check_arg(4)); // Too many arguments
        assert!(!cmd.check_arg(0)); // No arguments
    }
}
