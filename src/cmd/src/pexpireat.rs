...
impl PexpireatCmd {
    pub fn new() -> Self {
        Self {
            meta: CmdMeta {
                name: "pexpireat".to_string(),
                arity: 3, // PEXPIREAT key milliseconds-timestamp
...
impl Cmd for PexpireatCmd {
    ...
    /// the key will expire is specified in milliseconds since January 1, 1970.
    ///
    /// # Returns
    ...
    fn do_cmd(&self, client: &Client, storage: Arc<Storage>) {
        ...
        match storage.pexpireat(&key, timestamp) {
            Ok(success) => {
    ...
    fn execute_typed(&self, client: &Client, storage: Arc<Storage>) -> Option<CommandResult> {
        ...
        Some(match argv.len() {
            3 => match String::from_utf8_lossy(&argv[2]).parse::<i64>() {
                Ok(timestamp) => match storage.pexpireat(&argv[1], timestamp) {
