...
pub struct ZrevrangebyscoreCmd {
    meta: CmdMeta,
}

impl ZrevrangebyscoreCmd {
    pub fn new() -> Self {
        Self {
            meta: CmdMeta {
                name: "zrevrangebyscore".to_string(),
                arity: -4, // ZREVRANGEBYSCORE key max min [WITHSCORES] [LIMIT offset count]
...
impl Cmd for ZrevrangebyscoreCmd {
    ...
    fn do_cmd(&self, client: &Client, storage: Arc<Storage>) {
        ...
        let max_score = match String::from_utf8_lossy(&argv[2]).parse::<f64>() {
            Ok(s) => s,
            Err(_) => {
        ...
        let min_score = match String::from_utf8_lossy(&argv[3]).parse::<f64>() {
            Ok(s) => s,
            Err(_) => {
                client.set_error(error_catalog::MIN_MAX_NOT_FLOAT);
                return;
        ...
        let result =
            storage.zrevrangebyscore(&key, max_score, min_score, with_scores, offset, count);
