...
impl Cmd for SunionCmd {
    ...
    fn do_cmd(&self, client: &Client, storage: Arc<Storage>) {
        ...
        let result = storage.sunion(&keys);
...
mod tests {
    ...
    fn test_sunion_cmd_meta() {
        ...
        assert_eq!(cmd.meta().arity, -2); // SUNION key [key ...]
    ...
    #[test]
    fn test_sunion_argument_validation() {
        let cmd = SunionCmd::new();

        // Valid argument counts (2 or more: command + key1 [+ key2 ...])
        assert!(cmd.check_arg(2)); // SUNION key1
        assert!(cmd.check_arg(3)); // SUNION key1 key2
        assert!(cmd.check_arg(4)); // SUNION key1 key2 key3
