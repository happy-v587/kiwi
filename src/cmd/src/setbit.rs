    ...
    fn execute_typed(&self, client: &Client, storage: Arc<Storage>) -> Option<CommandResult> {
        ...
        Some(match argv.len() {
            4 => match (
                ...
                (Ok(offset), Ok(value)) => storage
                    .setbit(&argv[1], offset, value)
                    .map(RespData::Integer)
                    .map_err(crate::error::CommandError::storage),
                (Err(_), _) => Err(crate::error::CommandError::InvalidArgument(
                    crate::error::ArgumentError::NotInteger,
                )),
                (_, Err(_)) => Err(crate::error::CommandError::InvalidArgument(
                    crate::error::ArgumentError::BitNotInteger,
                )),
            },
