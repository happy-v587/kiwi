// Copyright (c) 2024-present, arana-db Community.  All rights reserved.
//
// Licensed to the Apache Software Foundation (ASF) under one or more
// contributor license agreements.  See the NOTICE file distributed with
// this work for additional information regarding copyright ownership.
// The ASF licenses this file to You under the Apache License, Version 2.0
// (the "License"); you may not use this file except in compliance with
// the License.  You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

use std::sync::Arc;

use bytes::Bytes;
use client::Client;
use resp::RespData;
use storage::storage::Storage;

use crate::{AclCategory, ClientExt, Cmd, CmdFlags, CmdMeta, CommandResult};
use crate::{impl_cmd_clone_box, impl_cmd_meta};

#[derive(Clone, Default)]
pub struct SdiffCmd {
    meta: CmdMeta,
}

impl SdiffCmd {
    pub fn new() -> Self {
        Self {
            meta: CmdMeta {
                name: "sdiff".to_string(),
                arity: -2, // SDIFF key [key ...]
                flags: CmdFlags::READONLY | CmdFlags::FAST,
                acl_category: AclCategory::SET | AclCategory::READ,
                ..Default::default()
            },
        }
    }
}

impl Cmd for SdiffCmd {
    impl_cmd_meta!();
    impl_cmd_clone_box!();

    fn do_initial(&self, client: &Client) -> bool {
        let argv = client.argv();
        if argv.len() < 2 {
            return false;
        }

        // Set the first key as the primary key for locking
        let key = argv[1].clone();
        client.set_key(&key);
        true
    }

    fn do_cmd(&self, client: &Client, storage: Arc<Storage>) {
        let argv = client.argv();

        // Convert arguments to byte slices
        let keys: Vec<&[u8]> = argv[1..].iter().map(|k| k.as_slice()).collect();

        let result = storage.sdiff(&keys);

        match result {
            Ok(members) => {
                // Convert Vec<String> to Vec<RespData>
                let resp_members: Vec<RespData> = members
                    .into_iter()
                    .map(|member| RespData::BulkString(Some(member.into_bytes().into())))
                    .collect();

                client.set_reply(RespData::Array(Some(resp_members)));
            }
            Err(e) => {
                client.set_storage_error(&e);
            }
        }
    }

    fn execute_typed(&self, client: &Client, storage: Arc<Storage>) -> Option<CommandResult> {
        let argv = client.argv();
        Some(if argv.len() < 2 {
            Err(crate::error::CommandError::WrongArity {
                command: self.name().to_string(),
            })
        } else {
            let keys = argv[1..].iter().map(Vec::as_slice).collect::<Vec<_>>();
            storage
                .sdiff(&keys)
                .map(|members| {
                    RespData::Array(Some(
                        members
                            .into_iter()
                            .map(|member| RespData::BulkString(Some(Bytes::from(member))))
                            .collect(),
                    ))
                })
                .map_err(crate::error::CommandError::storage)
        })
    }

    fn uses_typed_execution(&self) -> bool {
        true
    }
}

#[allow(clippy::unwrap_used)]
#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_sdiff_cmd_meta() {
        let cmd = SdiffCmd::new();
        assert_eq!(cmd.name(), "sdiff");
        assert_eq!(cmd.meta().arity, -2); // SDIFF key [key ...]
        assert!(cmd.has_flag(CmdFlags::READONLY));
        assert!(cmd.has_flag(CmdFlags::FAST));
        assert!(!cmd.has_flag(CmdFlags::WRITE));
    }

    #[test]
    fn test_sdiff_cmd_clone() {
        let cmd = SdiffCmd::new();
        let cloned = cmd.clone_box();
        assert_eq!(cloned.name(), cmd.name());
        assert_eq!(cloned.meta().arity, cmd.meta().arity);
    }

    #[test]
    fn test_sdiff_acl_category() {
        let cmd = SdiffCmd::new();
        assert!(cmd.acl_category().contains(AclCategory::SET));
        assert!(cmd.acl_category().contains(AclCategory::READ));
    }

    #[test]
    fn test_sdiff_argument_validation() {
        let cmd = SdiffCmd::new();

        // Valid argument counts (2 or more: command + key1 [+ key2 ...])
        assert!(cmd.check_arg(2)); // SDIFF key1
        assert!(cmd.check_arg(3)); // SDIFF key1 key2
        assert!(cmd.check_arg(4)); // SDIFF key1 key2 key3

        // Invalid argument counts
        assert!(!cmd.check_arg(1)); // Missing key
        assert!(!cmd.check_arg(0)); // No arguments
    }
}
