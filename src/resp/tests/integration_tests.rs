...
fn test_resp3_round_trip_map() {
    let original_data = RespData::Map(vec![
        (
            RespData::SimpleString(Bytes::from("key1")),
            RespData::Integer(42),
        ),
        (
            RespData::SimpleString(Bytes::from("key2")),
            RespData::Boolean(true),
        ),
        (
            RespData::SimpleString(Bytes::from("key3")),
            #[allow(clippy::approx_constant)]
    ...
    match response {
        RespData::Array(Some(items)) => {
            ...
            let proto_index = items
                .iter()
                .position(|item| item.as_string() == Some("proto".to_string()))
    ...
    match response {
        RespData::Map(pairs) => {
            ...
            let proto_pair = pairs
                .iter()
                .find(|(key, _)| key.as_string() == Some("proto".to_string()))
                .expect("proto field not found");
...
fn test_resp3_to_resp2_conversion() {
    ...
    let test_cases = vec![
        ...
        (
            #[allow(clippy::approx_constant)]
            RespData::Double(3.14),
            RespData::BulkString(Some(Bytes::from("3.14"))),
        ),
        (
            RespData::BigNumber(Bytes::from("999")),
            RespData::BulkString(Some(Bytes::from("999"))),
        ),
        (
...
fn test_auto_version_detection() {
    ...
    assert!(matches!(
        result,
        RespParseResult::Complete(RespData::Boolean(true))
    ));
}
